...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        # Ensure GPT-2 Model can process inputs_embeds
        try:
            # Check input shape and ensure it's compatible
            llm_input = enc_out.view(-1, enc_out.size(-1))  # Reshape for LLM input
            llm_output = self.llm(inputs_embeds=llm_input).last_hidden_state  # [B*S, D]
            llm_output = llm_output.view(enc_out.size())  # Reshape back to [B, S, D]
            
            # Combine LLM output with encoder output
            enhanced_enc_out = enc_out + llm_output
        except Exception as e:
            print("Error during LLM forward pass:", e)
            print("Encoder output shape:", enc_out.shape)
            print("LLM input shape:", llm_input.shape)
            raise

        dec_out = self.dec_embedding(x_dec_new, x_mark_dec)
        dec_out = self.decoder(dec_out, enhanced_enc_out, x_mask=dec_self_mask, cross_mask=dec_enc_mask, tau=tau, delta=delta)
